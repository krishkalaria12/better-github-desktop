...
fn save_repos_in_store(folder_path: String, app: tauri::AppHandle) -> Result<()> {
...
#[command]
pub fn check_is_git_repo(path: Option<String>, app: tauri::AppHandle) -> bool {
...
#[command]
pub fn get_last_opened_repo(app: tauri::AppHandle) -> Result<String> {
    let store = app.store(&config().STORE_NAME)?;
